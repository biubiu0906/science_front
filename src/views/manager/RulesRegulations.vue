<template>
  <div class="rules-page">
    ...
    <div v-show="data.advancedVisible" class="advanced-panel">
      <el-select v-model="data.advancedFilters.regulationLevel" placeholder="制定级别" clearable>
        <el-option v-for="level in regulationLevelOptions" :key="level" :label="level" :value="level" />
      </el-select>
      <el-date-picker
    ...
    <div class="table-card" v-loading="data.loading">
      ...
      >
        ...
        <el-table-column prop="regulationName" label="制度名称" min-width="720" sortable show-overflow-tooltip />
        <el-table-column prop="publishDate" label="颁布时间" width="164" sortable />
        <el-table-column prop="regulationLevel" label="制定级别" width="210" sortable show-overflow-tooltip />
    ...
    <el-dialog
      ...
      :title="data.form.id ? '编辑规章制度' : '新增规章制度'"
      width="760px"
      destroy-on-close
    >
      <el-form ref="formRef" :model="data.form" :rules="rules" label-width="118px" class="rules-form">
        <el-row :gutter="16">
          <el-col :span="12">
        ...
        <el-form-item label="制度名称" prop="regulationName">
          <el-input v-model="data.form.regulationName" type="textarea" :rows="2" placeholder="请输入制度名称" clearable />
        </el-form-item>
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="制定级别" prop="regulationLevel">
              <el-select v-model="data.form.regulationLevel" placeholder="请选择制定级别" style="width: 100%" filterable allow-create>
                <el-option v-for="level in regulationLevelOptions" :key="level" :label="level" :value="level" />
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-form-item label="制度内容" prop="content">
          <el-input v-model="data.form.content" type="textarea" :rows="5" placeholder="请输入制度内容" clearable />
        </el-form-item>
      </el-form>
      <template #footer>
...
const regulationLevelOptions = ['基地制度', '学校颁发的制度', '省部级制度', '国家级制度', '其他']

const formRef = ref()

const rules = {
  regulationName: [{ required: true, message: '请输入制度名称', trigger: 'blur' }],
  publishDate: [{ required: true, message: '请选择颁布时间', trigger: 'change' }],
  regulationLevel: [{ required: true, message: '请选择制定级别', trigger: 'change' }]
}
...
const normalizeRecord = (row = {}) => {
  return {
    ...
    regulationName: row.regulationName ?? row.regulation_name,
    publishDate: row.publishDate ?? row.publish_date,
    regulationLevel: row.regulationLevel ?? row.regulation_level,
...
const toApiPayload = (row = {}) => {
  return {
    ...
    regulation_name: row.regulationName,
    publish_date: row.publishDate,
    regulation_level: row.regulationLevel,
...
const load = () => {
  data.loading = true
  request.get('/rulesRegulations/selectPage', {
    params: {
      pageNum: data.pageNum,
      pageSize: data.pageSize,
      keyword: data.keyword || undefined,
      regulationLevel: data.advancedFilters.regulationLevel || undefined,
...
const resetSearch = () => {
  data.keyword = ''
  data.advancedFilters.regulationLevel = ''
...
const save = () => {
  formRef.value.validate(valid => {
    ...
    const api = data.form.id
      ? request.put('/rulesRegulations/update', payload)
      : request.post('/rulesRegulations/add', payload)
    api.then(res => {
      if (res.code === '200') {
...
const handleDelete = (row) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗？', '删除确认', { type: 'warning', buttonSize: 'small' }).then(() => {
    request.delete(`/rulesRegulations/delete/${row.id}`).then(res => {
      if (res.code === '200') {
...
const deleteSelected = () => {
  ...
  ElMessageBox.confirm(`确定删除已选择的 ${ids.length} 项数据吗？`, '删除确认', { type: 'warning', buttonSize: 'small' }).then(() => {
    request.delete('/rulesRegulations/delete/batch', { data: ids }).then(res => {
      if (res.code === '200') {
        ElMessage.success('删除成功')
        data.selectedRows = []
        load()
