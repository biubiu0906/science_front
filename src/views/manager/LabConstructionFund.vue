<template>
  <div class="fund-page">
    <header class="page-heading">
      <h2>实验室建设经费</h2>
      <p>本机构实际到账，用于项目研究、机构建设、办公支出等用途的各类经费。请根据实际到账的每笔资金录入</p>
    </header>
    ...
    <div v-show="data.advancedVisible" class="advanced-panel">
      <el-select v-model="data.advancedFilters.fundCategory" placeholder="经费类别" clearable>
        <el-option v-for="category in fundCategoryOptions" :key="category" :label="category" :value="category" />
      </el-select>
      <el-input
        v-model="data.advancedFilters.fundSource"
        placeholder="经费来源"
        clearable
        @keyup.enter="handleSearch"
      />
      <el-input
        v-model="data.advancedFilters.relatedProject"
        placeholder="所属项目"
        clearable
        @keyup.enter="handleSearch"
      />
    ...
    <div class="table-card" v-loading="data.loading">
      ...
      >
        ...
        <el-table-column prop="fundName" label="经费名称" min-width="400" sortable show-overflow-tooltip />
        <el-table-column prop="amount" label="金额(万元)" width="160" sortable align="right">
          <template #default="{ row }">{{ formatAmount(row.amount) }}</template>
        </el-table-column>
        <el-table-column prop="fundCategory" label="经费类别" width="150" sortable show-overflow-tooltip />
        <el-table-column prop="fundSource" label="经费来源" width="220" sortable show-overflow-tooltip />
        <el-table-column prop="receivedDate" label="到账日期" width="164" sortable />
        <el-table-column prop="relatedProject" label="所属项目" min-width="400" sortable show-overflow-tooltip />
        <el-table-column prop="attachmentCount" label="附件数" width="132" align="center">
    ...
    <el-dialog
      ...
      :title="data.form.id ? '编辑实验室建设经费' : '新增实验室建设经费'"
      width="820px"
      destroy-on-close
    >
      <el-form ref="formRef" :model="data.form" :rules="rules" label-width="118px" class="fund-form">
        <el-row :gutter="16">
          <el-col :span="12">
        ...
        <el-form-item label="经费名称" prop="fundName">
          <el-input v-model="data.form.fundName" type="textarea" :rows="2" placeholder="请输入经费名称" clearable />
        </el-form-item>
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="经费类别" prop="fundCategory">
              <el-select v-model="data.form.fundCategory" placeholder="请选择经费类别" style="width: 100%" filterable allow-create>
                <el-option v-for="category in fundCategoryOptions" :key="category" :label="category" :value="category" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="经费来源" prop="fundSource">
              <el-input v-model="data.form.fundSource" placeholder="请输入经费来源" clearable />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="到账日期" prop="receivedDate">
              <el-date-picker
                v-model="data.form.receivedDate"
                type="date"
                value-format="YYYY-MM-DD"
                placeholder="请选择日期"
                style="width: 100%"
        ...
        <el-form-item label="所属项目" prop="relatedProject">
          <el-input v-model="data.form.relatedProject" type="textarea" :rows="2" placeholder="请输入所属项目" clearable />
        </el-form-item>
...
const fundCategoryOptions = ['项目经费', '机构建设经费', '办公经费', '设备购置经费', '运行维护经费', '其他']

const formRef = ref()

const rules = {
  fundName: [{ required: true, message: '请输入经费名称', trigger: 'blur' }],
  amount: [{ required: true, message: '请输入金额', trigger: 'change' }],
  fundCategory: [{ required: true, message: '请选择经费类别', trigger: 'change' }],
  fundSource: [{ required: true, message: '请输入经费来源', trigger: 'blur' }],
  receivedDate: [{ required: true, message: '请选择到账日期', trigger: 'change' }]
}
...
const normalizeRecord = (row = {}) => {
  return {
    ...
    fundName: row.fundName ?? row.fund_name,
    amount: row.amount,
    fundCategory: row.fundCategory ?? row.fund_category,
    fundSource: row.fundSource ?? row.fund_source,
    receivedDate: row.receivedDate ?? row.received_date,
    relatedProject: row.relatedProject ?? row.related_project,
...
const toApiPayload = (row = {}) => {
  return {
    ...
    amount: row.amount,
    fund_category: row.fundCategory,
    fund_source: row.fundSource,
    received_date: row.receivedDate,
    related_project: row.relatedProject,
...
const load = () => {
  data.loading = true
  request.get('/labConstructionFund/selectPage', {
    params: {
      pageNum: data.pageNum,
      pageSize: data.pageSize,
      keyword: data.keyword || undefined,
      fundCategory: data.advancedFilters.fundCategory || undefined,
      fundSource: data.advancedFilters.fundSource || undefined,
      relatedProject: data.advancedFilters.relatedProject || undefined,
...
const resetSearch = () => {
  data.keyword = ''
  data.advancedFilters.fundCategory = ''
  data.advancedFilters.fundSource = ''
  data.advancedFilters.relatedProject = ''
...
const save = () => {
  formRef.value.validate(valid => {
    ...
    const api = data.form.id
      ? request.put('/labConstructionFund/update', payload)
      : request.post('/labConstructionFund/add', payload)
    api.then(res => {
      if (res.code === '200') {
...
const handleDelete = (row) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗？', '删除确认', { type: 'warning', buttonSize: 'small' }).then(() => {
    request.delete(`/labConstructionFund/delete/${row.id}`).then(res => {
      if (res.code === '200') {
...
const deleteSelected = () => {
  ...
  ElMessageBox.confirm(`确定删除已选择的 ${ids.length} 项数据吗？`, '删除确认', { type: 'warning', buttonSize: 'small' }).then(() => {
    request.delete('/labConstructionFund/delete/batch', { data: ids }).then(res => {
      if (res.code === '200') {
        ElMessage.success('删除成功')
        data.selectedRows = []
        load()
