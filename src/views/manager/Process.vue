  ...
  <div>
    ...
    <div class="card" style="margin-bottom: 5px">
      <div style="margin-bottom: 10px; margin-left: 10px;">
        ...
        <el-button v-if="data.user.role === 'SUPER_ADMIN' || data.user.role === 'SCHOOL_ADMIN'" type="danger" plain size="small" @click="delBatch">批量删除</el-button>
      </div>
      <el-table stripe :data="data.tableData" @selection-change="handleSelectionChange" :header-cell-style="{ backgroundColor: '#e9edf2' }" class="table-center" empty-text="暂无数据">
        <el-table-column v-if="data.user.role === 'SUPER_ADMIN' || data.user.role === 'SCHOOL_ADMIN'" type="selection" width="55" />
...
const handleEdit = (row) => {
  ...
  // 如果 projectId 为空，尝试根据 projectName 从 data.projectData 中匹配出 projectId，方便修改
  if (!data.form.projectId && data.form.projectName) {
    const project = data.projectData.find(p => p.name === data.form.projectName)
    if (project) {
      data.form.projectId = project.id
    }
  }
  data.formVisible = true
}
