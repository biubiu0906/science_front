<template>
  <div class="brief-page">
    ...
    <div v-show="data.advancedVisible" class="advanced-panel">
      <el-input v-model="data.advancedFilters.issueNo" placeholder="期次" clearable @keyup.enter="handleSearch" />
      <el-date-picker
    ...
    <div class="table-card" v-loading="data.loading">
      ...
      >
        ...
        <el-table-column prop="briefTitle" label="简报标题" min-width="720" sortable show-overflow-tooltip />
        <el-table-column prop="issueNo" label="期次" width="220" sortable show-overflow-tooltip />
    ...
    <el-dialog
      ...
      :title="data.form.id ? '编辑成果/工作简报' : '新增成果/工作简报'"
      width="760px"
      destroy-on-close
    >
      <el-form ref="formRef" :model="data.form" :rules="rules" label-width="118px" class="brief-form">
        <el-row :gutter="16">
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item label="期次" prop="issueNo">
              <el-input v-model="data.form.issueNo" placeholder="请输入期次" clearable />
            </el-form-item>
          </el-col>
        </el-row>
        <el-form-item label="简报标题" prop="briefTitle">
          <el-input v-model="data.form.briefTitle" type="textarea" :rows="2" placeholder="请输入简报标题" clearable />
        </el-form-item>
        <el-row :gutter="16">
          <el-col :span="12">
        ...
        <el-form-item label="简报内容" prop="content">
          <el-input v-model="data.form.content" type="textarea" :rows="5" placeholder="请输入简报内容" clearable />
        </el-form-item>
      </el-form>
      <template #footer>
...
const rules = {
  briefTitle: [{ required: true, message: '请输入简报标题', trigger: 'blur' }],
  issueNo: [{ required: true, message: '请输入期次', trigger: 'blur' }],
...
const normalizeRecord = (row = {}) => {
  return {
    ...
    briefTitle: row.briefTitle ?? row.brief_title,
    issueNo: row.issueNo ?? row.issue_no,
...
const load = () => {
  data.loading = true
  request.get('/workAchievementBrief/selectPage', {
    params: {
      pageNum: data.pageNum,
      pageSize: data.pageSize,
      keyword: data.keyword || undefined,
      issueNo: data.advancedFilters.issueNo || undefined,
...
const save = () => {
  formRef.value.validate(valid => {
    ...
    const api = data.form.id
      ? request.put('/workAchievementBrief/update', payload)
      : request.post('/workAchievementBrief/add', payload)
    api.then(res => {
      if (res.code === '200') {
...
const handleDelete = (row) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗？', '删除确认', { type: 'warning', buttonSize: 'small' }).then(() => {
    request.delete(`/workAchievementBrief/delete/${row.id}`).then(res => {
      if (res.code === '200') {
...
const deleteSelected = () => {
  ...
  ElMessageBox.confirm(`确定删除已选择的 ${ids.length} 项数据吗？`, '删除确认', { type: 'warning', buttonSize: 'small' }).then(() => {
    request.delete('/workAchievementBrief/delete/batch', { data: ids }).then(res => {
      if (res.code === '200') {
        ElMessage.success('删除成功')
        data.selectedRows = []
        load()
