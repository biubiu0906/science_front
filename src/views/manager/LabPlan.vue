<template>
  <div class="plan-page">
    ...
    <div v-show="data.advancedVisible" class="advanced-panel">
      <el-select v-model="data.advancedFilters.planPeriod" placeholder="规划周期" clearable>
        <el-option v-for="period in planPeriodOptions" :key="period" :label="period" :value="period" />
      </el-select>
      <el-date-picker
    ...
    <div class="table-card" v-loading="data.loading">
      ...
      >
        ...
        <el-table-column prop="planTitle" label="规划标题" min-width="720" sortable show-overflow-tooltip />
        <el-table-column prop="planPeriod" label="规划周期" width="164" sortable />
        <el-table-column prop="formulateDate" label="制定时间" width="164" sortable />
    ...
    <el-dialog
      ...
      :title="data.form.id ? '编辑实验室规划' : '新增实验室规划'"
      width="760px"
      destroy-on-close
    >
      <el-form ref="formRef" :model="data.form" :rules="rules" label-width="118px" class="plan-form">
        <el-row :gutter="16">
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item label="制定时间" prop="formulateDate">
              <el-date-picker
                v-model="data.form.formulateDate"
                type="date"
                value-format="YYYY-MM-DD"
                placeholder="请选择日期"
                style="width: 100%"
        ...
        <el-form-item label="规划标题" prop="planTitle">
          <el-input v-model="data.form.planTitle" type="textarea" :rows="2" placeholder="请输入规划标题" clearable />
        </el-form-item>
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="规划周期" prop="planPeriod">
              <el-select v-model="data.form.planPeriod" placeholder="请选择规划周期" style="width: 100%" filterable allow-create>
                <el-option v-for="period in planPeriodOptions" :key="period" :label="period" :value="period" />
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-form-item label="规划内容" prop="content">
          <el-input v-model="data.form.content" type="textarea" :rows="5" placeholder="请输入规划内容" clearable />
        </el-form-item>
      </el-form>
      <template #footer>
...
const planPeriodOptions = ['十五规划', '十一五规划', '十二五规划', '十三五规划', '十四五规划', '五年规划', '年度规划', '其他']

const formRef = ref()

const rules = {
  planTitle: [{ required: true, message: '请输入规划标题', trigger: 'blur' }],
  planPeriod: [{ required: true, message: '请选择规划周期', trigger: 'change' }],
  formulateDate: [{ required: true, message: '请选择制定时间', trigger: 'change' }]
}
...
const normalizeRecord = (row = {}) => {
  return {
    ...
    planTitle: row.planTitle ?? row.plan_title,
    planPeriod: row.planPeriod ?? row.plan_period,
    formulateDate: row.formulateDate ?? row.formulate_date,
...
const toApiPayload = (row = {}) => {
  return {
    ...
    formulate_date: row.formulateDate,
...
const load = () => {
  data.loading = true
  request.get('/labPlan/selectPage', {
    params: {
      pageNum: data.pageNum,
      pageSize: data.pageSize,
      keyword: data.keyword || undefined,
      planPeriod: data.advancedFilters.planPeriod || undefined,
...
const resetSearch = () => {
  data.keyword = ''
  data.advancedFilters.planPeriod = ''
...
const save = () => {
  formRef.value.validate(valid => {
    ...
    const api = data.form.id
      ? request.put('/labPlan/update', payload)
      : request.post('/labPlan/add', payload)
    api.then(res => {
      if (res.code === '200') {
...
const handleDelete = (row) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗？', '删除确认', { type: 'warning', buttonSize: 'small' }).then(() => {
    request.delete(`/labPlan/delete/${row.id}`).then(res => {
      if (res.code === '200') {
...
const deleteSelected = () => {
  ...
  ElMessageBox.confirm(`确定删除已选择的 ${ids.length} 项数据吗？`, '删除确认', { type: 'warning', buttonSize: 'small' }).then(() => {
    request.delete('/labPlan/delete/batch', { data: ids }).then(res => {
      if (res.code === '200') {
        ElMessage.success('删除成功')
        data.selectedRows = []
        load()
