    ...
    <div class="academic-shell">
      ...
      <section class="achievement-main">
        ...
        <div v-show="data.advancedVisible" class="advanced-panel">
          ...
          <el-input v-model="data.advancedFilters.journal" placeholder="发表期刊" clearable @keyup.enter="handleSearch" />
          <el-date-picker
        ...
        <div class="table-wrap" v-loading="data.loading">
          ...
          >
            ...
            <el-table-column prop="title" :label="titleColumnLabel" min-width="420" sortable show-overflow-tooltip />
            <el-table-column prop="journal" :label="journalColumnLabel" min-width="250" sortable show-overflow-tooltip />
            <el-table-column prop="publishDate" :label="dateColumnLabel" width="160" sortable />
            <el-table-column prop="authors" label="作者" min-width="260" show-overflow-tooltip />
            <el-table-column prop="institutionSignature" label="本机构署名" width="170" sortable />
            <el-table-column prop="attachment" label="附件" width="150" show-overflow-tooltip />
            <el-table-column prop="doi" label="DOI" min-width="180" show-overflow-tooltip />
            <el-table-column prop="indexedStatus" label="收录情况" width="150" />
...
const journalColumnLabel = computed(() => {
  ...
  return map[data.activeCategoryKey] || '来源'
})

const dateColumnLabel = computed(() => {
  return data.activeCategoryKey === 'journal' || data.activeCategoryKey === 'conference' ? '发表日期' : '完成日期'
})
...
const save = () => {
  formRef.value.validate(valid => {
    ...
    const api = data.form.id
      ? request.put('/academicAchievement/update', data.form)
      : request.post('/academicAchievement/add', data.form)
    api.then(res => {
      if (res.code === '200') {
